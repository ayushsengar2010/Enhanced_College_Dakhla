...
const AdminCollegeEnquiries = () => {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ["college-enquiries", search, statusFilter, page, limit],
    queryFn: () =>
      getLeads({
      ...
      <div className="flex items-center justify-between bg-white p-4 rounded-2xl border border-slate-200 shadow-sm flex-wrap gap-4">
        <div>
          <h1 className="text-xl font-black text-[#08162d] flex items-center gap-2">
            🏛️ College Page Enquiries
          </h1>
          <p className="text-xs text-slate-500 font-semibold mt-0.5">
            Leads submitted via the "Send Enquiry" form on individual college detail pages.
          </p>
        </div>
        <div className="flex items-center gap-3">
          ...
          <div className="bg-amber-50 border border-amber-200 px-4 py-2 rounded-xl text-xs font-black text-amber-700">
            Total: {total} leads
          </div>
          <button
            onClick={() => window.open("http://localhost:5000/api/leads/export/csv?source=college_detail", "_blank")}
          ...
          <table className="w-full text-left text-xs">
            <thead>
              <tr className="bg-slate-100/80 text-slate-700 border-b border-slate-200 font-extrabold uppercase tracking-wider">
                ...
                <th className="py-3.5 px-4">Course & City</th>
                <th className="py-3.5 px-4">College Enquired For</th>
                ...
                <tr>
                  <td colSpan={8} className="text-center py-14">
                    <div className="flex flex-col items-center gap-3 text-slate-400">
                      ...
                      <p className="font-bold text-sm">No college page enquiries yet.</p>
                      <p className="text-xs">When users fill the "Send Enquiry" form on a college page, they'll appear here.</p>
                    </div>
                  </td>
                </tr>
              ) : (
                leads.map((lead, i) => (
                  <tr key={lead._id} className="hover:bg-slate-50/60 transition-colors">
                    ...
                    <td className="py-3.5 px-4">
                      ...
                      <div className="text-[11px] text-slate-500">📍 {[lead.city, lead.state].filter(Boolean).join(", ") || "N/A"}</div>
                    </td>
                    <td className="py-3.5 px-4">
                      {lead.collegeId ? (
                        <div>
                          <div className="font-extrabold text-[#08162d] text-xs">
                            {lead.collegeId.collegeName || lead.collegeId.shortName || "—"}
                          </div>
                          {lead.collegeId.shortName && (
                            <div className="text-[10px] text-slate-400 font-semibold">{lead.collegeId.shortName}</div>
                          )}
                        </div>
                      ) : (
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl max-w-md w-full p-6 space-y-4 shadow-2xl border border-slate-200 relative">
            ...
            <div className="bg-slate-50 p-3 rounded-xl border border-slate-200 text-xs space-y-1">
              ...
              <p><strong>Course:</strong> {selectedLead.course || "N/A"} | <strong>City:</strong> {selectedLead.city || "N/A"}</p>
              {selectedLead.collegeId && (
                <p><strong>College:</strong> {selectedLead.collegeId.collegeName || "—"}</p>
              )}
              <p><strong>Source:</strong> <span className="text-amber-600 font-bold">College Detail Page</span></p>
            </div>
...
export default AdminCollegeEnquiries;
