...
const AdminHomeEnquiries = () => {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ["home-enquiries", search, statusFilter, page, limit],
    queryFn: () =>
      getLeads({
        source: "home_page",
  ...
  const updateMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries(["home-enquiries"]);
      ...
      <div className="flex items-center justify-between bg-white p-4 rounded-2xl border border-slate-200 shadow-sm flex-wrap gap-4">
        <div>
          ...
          <p className="text-xs text-slate-500 font-semibold mt-0.5">
            Leads submitted via the "Request Callback / Apply Online" form on the homepage.
          </p>
        </div>
        <div className="flex items-center gap-3">
          ...
          <div className="bg-blue-50 border border-blue-200 px-4 py-2 rounded-xl text-xs font-black text-blue-700">
            Total: {total} leads
          </div>
          <button
            onClick={() => window.open("http://localhost:5000/api/leads/export/csv?source=home_page", "_blank")}
                ...
                <tr>
                  <td colSpan={8} className="text-center py-14">
                    <div className="flex flex-col items-center gap-3 text-slate-400">
                      ...
                      <p className="font-bold text-sm">No home page enquiries yet.</p>
                      <p className="text-xs">When users fill the form on the homepage, they'll appear here.</p>
                    </div>
                  </td>
                </tr>
              ) : (
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl max-w-md w-full p-6 space-y-4 shadow-2xl border border-slate-200 relative">
            ...
            <div className="bg-slate-50 p-3 rounded-xl border border-slate-200 text-xs space-y-1">
              ...
              <p><strong>Source:</strong> <span className="text-blue-600 font-bold">Home Page</span></p>
            </div>
...
export default AdminHomeEnquiries;
