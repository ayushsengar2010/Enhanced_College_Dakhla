...
const AdminExams = () => {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ["admin-exams", search, page, limit],
    queryFn: () => getExams({ search: search || undefined, page, limit }),
  });
  ...
  const handleDelete = (id, name) => {
    if (window.confirm(`Are you sure you want to delete entrance exam "${name}"?`)) {
          ...
          <table className="w-full text-left text-xs">
            <thead>
              <tr className="bg-slate-100/80 text-slate-700 border-b border-slate-200 font-extrabold uppercase tracking-wider">
                ...
                <th className="py-3.5 px-4">Participating College ↕</th>
            ...
            <tbody className="divide-y divide-slate-200 text-slate-800 font-medium">
              {isLoading ? (
                ...
                <tr>
                  <td colSpan={9} className="text-center py-8 text-slate-400 font-bold">
                    No entrance exams found. Click "+ Add Entrance" to create one.
                  </td>
                </tr>
              ) : (
                exams.map((exam, index) => (
                  <tr key={exam._id} className="hover:bg-slate-50/80 transition-colors">
                    ...
                    <td className="py-3.5 px-4 font-black text-[#08162d]">{exam.examName}</td>
                    <td className="py-3.5 px-4 font-semibold text-slate-600">{fmtDate(exam.examDate)}</td>
                    <td className="py-3.5 px-4 font-semibold text-slate-700">{exam.examLevel || "National"}</td>
                    <td className="py-3.5 px-4 text-blue-600 truncate max-w-[180px]">
                      <a href={exam.applicationLink || exam.officialWebsite || "#"} target="_blank" rel="noopener noreferrer" className="hover:underline">
                        {exam.applicationLink || "https://www.collegedakhla.com/"}
                      </a>
                    </td>
                    <td className="py-3.5 px-4 text-center font-bold text-slate-700">{exam.participatingCollegesCount || 10}</td>
                    <td className="py-3.5 px-4 font-semibold text-slate-700">{exam.examMode || "Online"}</td>
                    <td className="py-3.5 px-4">
                      <span className={`font-black text-xs ${exam.status === "Active" ? "text-emerald-600" : "text-rose-500"}`}>
